...
export const AccountSelection = ({ accounts, selectedIds, onToggle }) => {
	const AccountAvatar = ({ src, platform }: { src?: string; platform: string }) => {
		...
		return (
			<div className="relative">
				...
				/>
				<img className="w-4 h-4 absolute bottom-0 right-0" src={icons[platform.toLowerCase()]} alt="" />
			</div>
		);
	};
						...
						{accounts.map((account) => (
							<div className="flex items-center justify-between" key={account.id}>
								<div className="flex items-center gap-2">
									<AccountAvatar src={account.profilePictureUrl} platform={account.platform} />
